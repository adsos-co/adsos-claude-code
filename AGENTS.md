...
- **The flow runs through the tools**: `get_status` then `get_intake_questions` then `submit_intake`, then a card step (the user adds a card at the /billing link the tools hand back: $0 that day, and nothing is charged unless they approve the finished report), then `get_audit_instructions`, `submit_audit`, `check_report`. Work through the intake with the user in plain conversation. Gather real numbers with the user's approval; never invent or estimate metrics on their behalf.
- **Pricing is public** (Base $95/mo with the first month at $76 on monthly, Ultimate $195/mo, annual saves 30%; full table at adsos.co/#pricing). State it plainly when asked; never pressure the user toward approval.
- **After the user approves their report** (picking a plan on the /approve page), this same connector runs the paid workers: `open_session`, `ask_director`, `ask_manager`, `run_specialist`. Nothing more to install.
